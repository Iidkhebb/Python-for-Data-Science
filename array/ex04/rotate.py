...
def rotate(path: str):
    ...
    transpose_image = np.transpose(cropped_image, axes=(1, 0, 2))  # Transpose height and width axes
    crop_height, crop_width, crop_ch = transpose_image.shape
    print(f"The shape of image is: {cropped_image.shape} or ({crop_height}, {crop_width})")
    print(cropped_image)
    print("New shape after transpose: ", f"({crop_height}, {crop_width})")
